...
def my_PCA(X_standard, num_components):
    ...
    X_reduced = np.dot(
        eigenvector_subset.transpose(), X_standard.transpose()
    ).transpose()
...
def plot_scatter(pc1, pc2, classes, figname):
    ...
    for i, spec in enumerate(classes):
        plt.scatter(
            ...
            label=spec,
            s=20,
            c=classes_colors[classes_unique.index(spec)],
        )
...
df_iono = dd.read_csv("ionosphere-dataset/ionosphere.data")


cols = [f"real_{i//2}" if i % 2 == 0 else f"imaginary_{i//2}" for i in range(0, 34)] + [
    "target"
]
...
iono_arr = df_iono.iloc[:, :-1].to_dask_array()


# dropping the target  column  (bad/good)
classes = df_iono["target"].to_dask_array()
...
print(
    "Explained variation per principal component: {}".format(
        pca_iono.explained_variance_ratio_
    )
)
...
X_standard_arr = X_standard_my.to_dask_array()

X_standard_arr[da.isnan(X_standard_arr)] = 0.0
...
print(
    "maximum difference found between my standardized dataset and the one standardized with scikit is:"
)
X_standard_arr.compute()
print(da.max(X_standard_arr - X_standard))
...
if abs(Myeuclide_dist_pair1 - Scipy_dist_pair1) < 1.0e-15:
    test_dist_res = "passed"

print("test distance: ", test_dist_res)
Euclidean_distance_arr = np.array(
    [
        Euclidean_dist(df_iono_reduced.iloc[0].values, df_iono_reduced.iloc[i].values)
        for i in range(df_iono_reduced.shape[0])
    ]
)
